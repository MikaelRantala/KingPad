...
import { getProjectDetails, getTotalSupply } from 'src/Contracts';
...
export const KingSale = () => {
  ...
  const getCoinData = async () => {
    ...
    if (data !== undefined) {
      setState({
        ...
        kingPassTotalSupply: res.toString(),
  ...
  const handleLaunch = () => {
    handleContractFunction(async () => await createPresale(), setLoad);
  };

  const handleTokenAddressChanged = async (value: string) => {
    console.log({ value });
    handleState('tokenAddress', value);
    console.log('handleTokenAddressChanged');
    try {
      const res = await getTokenInfo(value);
      setState({
        ...state,
        tokenAddress: value,
        name: res.tokenName,
        symbol: res.tokenSymbol,
        decimal: res.tokenDecimal
      });
    } catch (err) {
      console.log(err);
    }
  ...
  return (
    <>
      ...
      {isConnected ? (
        <LauchButton onClick={handleLaunch}>LAUNCH YOUR KING STARTER</LauchButton>
      ) : (
        <LaunchConnectButton />
      )}
    </>
