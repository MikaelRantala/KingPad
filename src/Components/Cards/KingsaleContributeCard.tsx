...
import { getUserContribution, presaleWithdraw, kingSaleDeposit } from 'src/Contracts/kingSale';
...
export const KingSaleContributeCard = (props: CardProps) => {
    ...
    if (minBuy !== undefined && maxBuy !== undefined && currency !== undefined) {
      if (buyVal < minBuy) {
        ...
        if (tokenAddress !== undefined) {
          ...
          handleContractFunction(async () => await kingSaleDeposit(currency, buyVal, tokenAddy), setLoad);
        }
      }
    }
  };
  ...
  const getContributeValue = async (userAddy: string) => {
    ...
    if (tokenAddy !== undefined) {
      ...
      const val = await getUserContribution(tokenAddy, userAddy);
      if (val !== undefined) {
        setContributeValue(parseFloat(val));
      }
    }
  };
  ...
  const handleWithdraw = async () => {
    const tokenAddy = isTest ? token_address : tokenAddress;
    if (tokenAddy !== undefined) {
      handleContractFunction(async () => await presaleWithdraw(), setLoad);
    }
  };
  ...
  return (
    <CardBox about="Contribute-Card" status={status}>
      ...
      <CardButtonGroup>
        ...
        <CardButton disabled={status !== 0} onClick={async () => await handleBuy()}>
          Buy
        </CardButton>
      </CardButtonGroup>
      <PurchasedContainer>
      ...
      {/* disabled={status !== 0} */}
      <WithdrawButton disabled={status !== 0} onClick={async () => await handleWithdraw()}>
        Withdraw
      </WithdrawButton>
    </CardBox>
  );
